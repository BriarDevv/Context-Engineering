...
- `name`: ≤64 chars, lowercase letters/numbers/hyphens. Gerund or
  action-oriented (`auditing-context`, `context-init`). Never vague
...
- `description`: ≤1024 chars, **third person**, states BOTH what the skill
  ...
  this field — Claude picks among 100+ skills using descriptions alone.
  - Good: "Audits a repository's Claude context files against the standard
    and reports fixes. Use when context feels bloated or after context-init."
  - Bad: "Helps with context stuff."

## Body rules

- SKILL.md body <500 lines; split beyond that.
- Only non-inferable content — Claude is already smart. Challenge every
  paragraph: does it justify its token cost?
- References exactly **one level deep** from SKILL.md (nested chains get
  partially read with `head` and information is lost).
...
4. Iterate from real usage: observe where the agent stumbles, fix the skill
   ...
   content, per this repo's CONTRIBUTING.md).
