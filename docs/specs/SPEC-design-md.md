...
cycle with its own context-init migration gate.
...
- Generator: `scripts/design-md-gen.mjs` in this repo — zero-dep Node, like
  context-lint. `node design-md-gen.mjs <DESIGN.md> --target tailwind4|cssvars`.
...
- context-lint, active only when a DESIGN.md exists in the scanned tree:
  ...
  - `design-ungenerated` (medium): frontmatter has tokens but no adjacent
    `design.tokens.css`.
- context-audit checklist, new "Design checks" section (judgment):
  ...
  - `## Decisions` entries contradicted by the current code (the disappearing
    back button) → high.
- context-init: when repo exploration detects a UI stack, offer DESIGN.md
...
| Repo | Artifact |
|---|---|
| Context-Engineering | `reference/design-md.md` (distilled spec + extension + compilation rule, sources cited) |
| Context-Engineering | `templates/repo/DESIGN.md.template` |
| Context-Engineering | `scripts/design-md-gen.mjs` + generator fixture tests |
| Context-Engineering | context-lint design checks + fixture tests; audit checklist rows; context-init UI-detection step |
